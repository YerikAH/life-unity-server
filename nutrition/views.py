...
class ValuesConsumedPerDayViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if id_user is not None:
            queryset = queryset.filter(id_user=id_user).filter(created_at__date=today)
        return queryset
    ...
    def create(self, request, *args, **kwargs):
        ...
        if value_consumed:
            ...
            if detail_instance:
                ...
                serializer = self.get_serializer(
                    detail_instance, data=mutable_data, partial=True)
                serializer.is_valid(raise_exception=True)
                self.perform_update(serializer)
                return Response(serializer.data, status=status.HTTP_200_OK)
        mutable_data.update({
        ...
        return super(ValuesConsumedPerDayViewSet, self).create(mutable_data, *args, **kwargs)
